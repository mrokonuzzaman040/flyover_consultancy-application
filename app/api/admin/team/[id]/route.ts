...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    await dbConnect();
    const doc = await Team.findById(params.id).lean();
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const doc = await Team.findByIdAndUpdate(params.id, parsed.data, { new: true }).lean();
    if (!doc) return NextResponse.json({ error: "Team member not found" }, { status: 404 });
    return NextResponse.json({ team: toObject(doc as { _id: unknown }) });
  } catch (e: unknown) {
    console.error(e);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    await dbConnect();
    const doc = await Team.findByIdAndDelete(params.id).lean();
    if (!doc) return NextResponse.json({ error: "Team member not found" }, { status: 404 });
