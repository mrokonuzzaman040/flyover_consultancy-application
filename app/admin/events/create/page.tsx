...
import { ArrowLeft, Plus, X, MapPin, Users, DollarSign, Calendar, Clock } from "lucide-react"
...
export default function CreateEventPage() {
  ...
  const updateArrayItem = (field: string, index: number, item: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: (prev as any)[field].map((existingItem: any, i: number) => 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (payload.targetAudience.length === 0) delete payload.targetAudience
      if (payload.requirements.length === 0) delete payload.requirements
      if (payload.agenda.length === 0) delete payload.agenda
      if (payload.speakers.length === 0) delete payload.speakers
      if (payload.tags.length === 0) delete payload.tags
      if (payload.materials.length === 0) delete payload.materials
