...
import { Award, Star, Trophy, Medal, Shield, Zap, Users, Lightbulb } from 'lucide-react';
import sectionsData from '@/data/sections-data.json';

interface AwardsSectionProps {
  awards?: typeof sectionsData.awards;
}

export default function AwardsSection({ awards = sectionsData.awards }: AwardsSectionProps) {
  ...
  // Remove hardcoded awards array - now using props
              ...
              <div className="relative mb-6 flex items-center justify-between">
                <div className={`w-16 h-16 bg-gradient-to-br from-brand to-brand-300 rounded-2xl flex items-center justify-center text-white group-hover:scale-105 transition-all duration-300 shadow-lg`}>
                  <div className="transform group-hover:scale-110 transition-transform duration-300">
                    {React.createElement(iconMap[award.icon as keyof typeof iconMap], { className: "w-8 h-8" })}
                  </div>
                </div>
                {/* Year badge */}
